...
export default async function ProjectPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="case-study">
    ...
    <section className="case-hero" style={{"--accent":project.accent} as React.CSSProperties}><span>{project.number} / {project.status}</span><h1>{project.title}</h1><p>{project.category}</p><div className="case-art"><i/><i/><i/><strong>{project.title}</strong></div></section>
